...
agrupamento = {
    ...
    "caroten": "carotenoid",
    "apocarotenes": "carotenoid",
    "apocarotenoid": "carotenoid",
    "apocarotenoids": "carotenoid",
    "caroteno": "carotenoid",
    "carotenoides": "carotenoid",
...
df_species["bioactives_grouped"] = df_species["bioactives_grouped"].apply(
    substituir_bioativos)
df_species.to_csv(r"C:\Users\Andrey\Desktop\Artigo_Maio\FIltros_Tree\Banco_Dados_Filtrado04.csv",
...
filtered_df02.fillna('')

filtered_df03 = df_species[['FUNG', 'Biosafety',
                            'bioactives']][mask_fam2 | mask_fam]
filtered_df03.fillna('')
...
# %%
k = df_species['bioactives_grouped'].dropna().str.split(
...
for _, linha in df_species.iterrows():
    if pd.notna(linha['family_Grouped']) and pd.notna(linha['bioactives_grouped']):
        familias = [f.strip() for f in linha['family_Grouped'].split(',')]
        bioativos = [b.strip() for b in linha['bioactives_grouped'].split(',')]
...
df.to_csv(
    r'C:\Users\Andrey\Desktop\Artigo_Maio\StreamLit\contagem_fam_bio_ModelosU.csv', index=False)
...
for _, linha in df_species.iterrows():
    if pd.notna(linha['Bacteria']) and pd.notna(linha['bioactives_grouped']) and pd.notna(linha['gram']):
...
df02.to_csv(
    r'C:\Users\Andrey\Desktop\Artigo_Maio\StreamLit\contagem_fam_bio_bacte_gram01.csv', index=False)
# %%
...
df03 = pd.DataFrame(columns=k)
df03.insert(0, 'FUNG', '')


for _, linha in df_species.iterrows():
    if pd.notna(linha['FUNG']) and pd.notna(linha['bioactives_grouped']):
        FUNG = [b.strip() for b in linha['FUNG'].split(',')]
        bioativos = [b.strip() for b in linha['bioactives_grouped'].split(',')]

        for fung in FUNG:
            if fung not in df03['FUNG'].values:
                nova_linha = dict.fromkeys(df03.columns, 0)
                nova_linha['FUNG'] = fung
                df03 = pd.concat([df03, pd.DataFrame([nova_linha])],
                                 ignore_index=True)

            for bio in bioativos:
                if bio in df03.columns:
                    df03.loc[df03['FUNG'] == fung, bio] += 1


df03.to_csv(
    r'C:\Users\Andrey\Desktop\Artigo_Maio\StreamLit\contagem_fam_bio_fung_bios01.csv', index=False)
# %%
